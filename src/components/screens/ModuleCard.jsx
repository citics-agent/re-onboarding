...
export const ModuleCard = ({
  ...
  if (content.slides) {
    content.slides = content.slides.map((s) =>
      s.startsWith("/") ? import.meta.env.BASE_URL + s.slice(1) : s
    );
  }
    ...
    >
      <Card className="max-w-5xl w-full mx-auto overflow-visible relative">
        ...
        <div className="mb-4 pr-16">
          ...
          <p className="text-slate-600 font-medium mt-1.5 text-sm leading-relaxed">
            {description}
          </p>
        </div>
          ...
          {showTip && (
            ...
            >
              ...
              <p className="text-xs font-medium text-slate-600">
                Mở{" "}
                <span className="text-citics-blue font-bold">
                  toàn màn hình
                </span>{" "}
                và{" "}
                <span className="text-citics-blue font-bold">xoay ngang</span>{" "}
                để xem tài liệu tốt nhất nhé!
              </p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex gap-3">
          <Button
            ...
            variant="outline"
            onClick={onBack}
            className="w-1/3 text-slate-600 border-slate-200 hover:bg-slate-50 px-2 text-sm md:text-base"
          >
            Quay lại
          </Button>
          <Button
