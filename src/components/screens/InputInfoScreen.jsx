...
export const InputInfoScreen = ({ onNext, onBack }) => {
    ...
    formFields.forEach((field) => {
      ...
      if (field.required) {
        if (field.type === "multiselect") {
          if (
            !formData[field.id] ||
            formData[field.id].length !== field.maxSelect
          ) {
            newErrors[field.id] =
              field.errorMessage || `Vui lòng chọn đủ ${field.maxSelect} mục`;
          }
    ...
    if (currentPage < TOTAL_PAGES) {
      ...
      if (validatePage(currentPage)) {
        ...
        const visibleFieldIds = new Set(
          formFields.filter(isFieldVisible).map((f) => f.id)
        );
  ...
  const inputClass = (id, isSelect = false) =>
    `w-full bg-slate-50 border rounded-xl px-3.5 py-2.5 text-sm ${isSelect && !formData[id] ? "text-slate-400" : "text-slate-900"} placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-offset-0 transition-all ${
      errors[id]
          ...
          >
            {currentFields.map((field, index) => (
              <div key={index} className="relative">
                  ...
                  {field.tooltip && (
                    <span className="relative inline-flex self-center ml-0.5">
                      <button
                        ...
                        onClick={() =>
                          setActiveTooltip(
                            activeTooltip === field.id ? null : field.id
                          )
                        }
                ...
                {field.type === "multiselect" ? (
                  <div
                    className={`${field.maxSelect ? "max-h-40 overflow-y-auto pr-1" : ""} rounded-xl custom-scrollbar border border-slate-100 p-2 bg-slate-50/50`}
                  >
                    <div
                      className={`flex flex-col gap-2 ${field.maxSelect ? "grid grid-cols-2 md:grid-cols-3" : ""} ${!field.maxSelect ? "[&>button]:text-left [&>button]:pl-5" : ""}`}
                    >
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (field.maxSelect === 1) {
                                ...
                                if (
                                  field.maxSelect &&
                                  current.length >= field.maxSelect
                                )
                                  return;
                                next = [...current, option];
                              }
            ...
            <div className="pt-2 flex gap-3">
              <Button
                ...
                className="w-1/3 text-slate-600 border-slate-200 hover:bg-slate-50 text-sm md:text-base px-2"
              >
                Quay lại
              </Button>
